...
std::vector<unsigned char*> load_images(const cv::Mat &l_img, const cv::Mat &f_img, const cv::Mat &b_img, const cv::Mat &r_img);
...
class BEVFusionNode
{
 ...
 public:
	...
	std::vector<bevfusion::head::transbbox::BoundingBox> Inference(const std::vector<unsigned char *>& images_data, float *lidar_arr, int lidar_num);
