...
{
  const char* file_names[] = {"0-left.png", "1-front.png", "2-backward.png", "3-right.png"};
...
std::vector<unsigned char*> load_images(
  const cv::Mat &l_img, const cv::Mat &f_img, const cv::Mat &b_img, const cv::Mat &r_img)
{
  std::vector<unsigned char*> _images;
  _images.push_back(cv2stb(l_img));
  _images.push_back(cv2stb(f_img));
  _images.push_back(cv2stb(b_img));
  _images.push_back(cv2stb(r_img));
  return _images;
}
...
std::shared_ptr<bevfusion::Core> create_core(const std::string& model, const std::string& precision)
{
  ...
  normalization.image_height = 800;
  ...
  param.fuser = pkg_path + nv::format("/model/%s/build/fuser.plan", model.c_str());
  param.head = pkg_path + nv::format("/model/%s/build/head.bbox.plan", model.c_str());
...
std::vector<bevfusion::head::transbbox::BoundingBox> BEVFusionNode::Inference(const std::vector<unsigned char *>& images_data, float *lidar_arr, int lidar_num)
{
...
void BEVFusionNode::visualize(const std::vector<bevfusion::head::transbbox::BoundingBox>& bboxes, const nv::Tensor& lidar_points,
  ...
  bev_artist_param.rotate_x = 0.0f; // 70.0f
  bev_artist_param.rotate_z = 90.0f; // 10.0f
  ...
  bev_visualizer->draw_prediction(filtered_predictions, false);
  ...
  image_artist_param.image_height = 800;
  ...
  int offset_cameras[][3] = {
      ...
      // {content_width / 2 - camera_width - gap, - content_height / 2 + camera_height + offset, 0},
      // {-content_width / 2 + gap, - content_height / 2 + camera_height + offset, 0},
  ...
  for (size_t icamera = 0; icamera < images.size(); ++icamera)
  {
    ...
    visualizer->draw_prediction(icamera, filtered_predictions, xflip);

    nv::Tensor device_image(std::vector<int>{800, 1280, 3}, nv::DataType::UInt8);
