...
#include "head-anchor.hpp"

namespace bevfusion {
namespace head {

class HeadImplement : public AnchorHead {
 public:
  virtual ~HeadImplement() {
  ...
  virtual void print() override { engine_->print("Head"); }

  virtual std::vector<std::vector<int>> output_shape() override { return bindshape_; }

  virtual BindingOut forward(const nvtype::half* fusion_bev, void* stream) override {
    cudaStream_t _stream = static_cast<cudaStream_t>(stream);

    engine_->forward({/* input  */ fusion_bev,
...
std::shared_ptr<AnchorHead> create_head(const std::string& param) {
  std::shared_ptr<HeadImplement> instance(new HeadImplement());
